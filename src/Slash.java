/**
 * Slash class searches on right diagonal direction to found triple
 * Calls the methods from RightDiagonal class for searching and shifting operations
 * Slash object only matches with slash characters.
 * */
...
public class Slash extends Jewel {
    ...
    public ArrayList<ArrayList<Jewel>> shift(int row, int column, ArrayList<ArrayList<Jewel>> gridMapList, LeaderBoard player, Grid grid) {
        int[] coordinates = rightDiagonal.findMathCoordinates(row, column, gridMapList);    //Calls the method for searching operation
